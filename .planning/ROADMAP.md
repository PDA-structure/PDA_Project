...
- [x] 999.2-04-PLAN.md — Wave 4: UI combination table + two-page generator wizard + (i) info-popovers (engine-driven; JS superpose on edit, no re-solve) [D-08,09,14,15,16,17]
- [x] 999.2-05-PLAN.md — Wave 5: UI results selector (per-case/per-combination/ENVELOPE) + governing column + canvas reverse-index highlight + per-member export string (schema 1.2) [D-18,19,20,21]
