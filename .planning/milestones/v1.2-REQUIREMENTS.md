# Requirements Archive: v1.2 2D Frame Hardening + Revit-as-UI (MVP)

**Archived:** 2026-04-26
**Status:** SHIPPED — all 13 requirements satisfied
...
- [x] **PUREBAR-01**: `frame_v2.assemble_primary_stiffness_matrix` detects pure-bar joints (every incident member is a bar) during assembly
- [x] **PUREBAR-02**: Hybrid beam+bar models (e.g. Pratt/Warren trusses with continuous beam chords) solve correctly; pure-bar joints no longer cause `Ks` singularity / HTTP 422
- [x] **PUREBAR-03**: `frame_v2.apply_equivalent_nodal_actions` no longer silently drops UDL applied to bar members — either applied correctly as nodal forces, or rejected with a clear error
- [x] **PUREBAR-04**: `frame2d` UI replaces generic "Structure is unstable" with specific diagnostics; canvas highlights joints with zero rotational stiffness; flag dropped UDL on bar members
- [x] **PUREBAR-05**: Regression test added using the captured failing fixture (`~/Downloads/frame2d-model-2026-04-22T06-14-49.json`)

## Bonus Scope (delivered outside formal requirements)

- **Quick task 260423-a0q** — pyRevit `ExportToPDA_Truss` pushbutton in sibling `CustomRevitExtension` repo. Clones the Phase 5 frame2d exporter and emits truss2d-schema JSON (drafting-view detail lines → canonical truss2d JSON). Built 2026-04-23, HUMAN-UAT round-trip passed 2026-04-24. Opportunistic scope outside the formal v1.2 requirements list.
...
| REVIT-T1-01 | Phase 5 | Complete |
| REVIT-T1-02 | Phase 5 | Complete |
| REVIT-T1-03 | Phase 5 | Complete |
| REVIT-T1-04 | Phase 5 | Complete |
| REVIT-T1-05 | Phase 5 | Complete |
| PUREBAR-01 | Phase 6 | Complete |
| PUREBAR-02 | Phase 6 | Complete |
| PUREBAR-03 | Phase 6 | Complete |
| PUREBAR-04 | Phase 6 | Complete |
| PUREBAR-05 | Phase 6 | Complete |
| REVIT-T2-01..07 | Deferred to v1.3 | — |

_Phase mapping filled in 2026-04-19 after roadmap creation. Phase 6 rescoped 2026-04-26: Revit Tier 2 deferred to v1.3, pure-bar joint robustness promoted from backlog. All v1.2 requirements verified satisfied at milestone close 2026-04-26 (`.planning/milestones/v1.2-MILESTONE-AUDIT.md`)._
